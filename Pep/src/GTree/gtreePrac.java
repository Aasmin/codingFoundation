...
//    	for(Node child : root.children) {		//this gives an error
//    	for(int i = 0; i < root.children.size(); i++) {		// if node is removed then size also changes
    	for(int i = root.children.size() - 1; i >= 0; i--) {	//it is better to start from last element
    		Node child = root.children.get(i);
    		if(child.children.size() == 0){
    			root.children.remove(child);
//    			i--;	//do this if starting from i = 0
    		}
    		else
    			removeLeaves(child);
    	}
    ...
    static boolean isIsomorphic(Node root1, Node root2) {
    	if(root1.children.size() != root2.children.size()) {
    		...
    		// that means both the root have some number of child
...
//         display(root);
         ...
         int[] dlist1 = {
        		 1, 2, -1, 3, -1, 4, 5, -1, 6, 7, -1, 8, -1, 9, -1, -1, -1, -1
         };
         Node root1 = build(dlist1);
         
         int[] dlist2 = {
        		 10, 20, -1, 30, -1, 40, 50, -1, 60, 70, -1, 80, -1, 90, -1, -1, -1, -1
         };
